...
const port = 9341;
const profile = path.resolve('.chrome-parent-home-capture');
const outputDir = path.resolve('../../screenshots/design-system-v1/implementation');
const pageUrl = 'http://127.0.0.1:4173/parent/dashboard';
...
try {
  ...
  await capture(client, { width: 1440, height: 1024, mobile: false, file: 'parent-home-desktop-1440.png' });
  await capture(client, { width: 1024, height: 1024, mobile: false, file: 'parent-home-tablet-1024.png' });
  await capture(client, { width: 375, height: 812, mobile: true, file: 'parent-home-mobile-375.png' });
  client.close();
} finally {
  chrome.kill();
  await sleep(300);
