...
const port = 9337;
const profile = path.resolve('.chrome-task-capture');
const outputDir = path.resolve('../../screenshots/design-system-v1/implementation');
const pageUrl = 'http://127.0.0.1:4173/child/tasks';
...
try {
  ...
  await capture(client, { width: 1440, height: 1024, mobile: false, file: 'task-desktop-1440.png' });
  await capture(client, { width: 1024, height: 1024, mobile: false, file: 'task-tablet-1024.png' });
  await capture(client, { width: 375, height: 812, mobile: true, file: 'task-mobile-375.png' });
  client.close();
} finally {
  chrome.kill();
  await sleep(300);
