-- Ensure approving a screen-time redemption creates a distinct tablet_time log
-- row. Migration 034 allowed callers to send the same client_request_id for the
-- request and log; that made the log insert conflict with the request row.
...
declare
  ...
  v_log_key text := coalesce(nullif(coalesce(p_log ->> 'idempotency_key', ''), ''), nullif(coalesce(p_log ->> 'client_request_id', ''), ''), 'screen-time-request:' || v_request_id::text || ':tablet-time');
