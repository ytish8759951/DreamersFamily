-- Replace create_family_for_current_user with a signature that avoids family_id output-name ambiguity.
-- Returns camelCase RPC fields while keeping database PKs as UUIDs.
...
returns table("familyId" uuid, "parentId" uuid, "parentRole" text, "familyCode" text)
language plpgsql
security definer
...
begin
  ...
  if v_existing_family_id is not null then
    ...
    return query
      select
        v_existing_family_id as "familyId",
        v_parent_id as "parentId",
        v_parent_role as "parentRole",
        v_family_code as "familyCode";
    return;
  end if;
